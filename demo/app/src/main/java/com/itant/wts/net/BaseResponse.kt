...
class BaseResponse<T> : NetResponse {
    ...
    override fun valid(): Boolean {
        if (code != 200) {
            ...
            /*if (code == 401 && UserManager.userLoggedIn) {
                SettingManager.onLogout()
                GlobalContext.getContext()?.let {
                    val loginIntent = Intent(it, HomeActivity::class.java).apply {
                        flags = Intent.FLAG_ACTIVITY_NEW_TASK
                        putExtra(HomeActivity.KEY_FOR_LOGIN, true)
                    }
                    it.startActivity(loginIntent)
                }
                return false
            }*/
            // 如果要关闭界面，在AndroidManifest.xml中设置android:launchMode="singleTask"
            /*HomeActivity处理：
            override fun onNewIntent(intent: Intent) {
                super.onNewIntent(intent)
                val forLogin = intent.getBooleanExtra(KEY_FOR_LOGIN, false)
                if (forLogin) {
                    ToastUtils.showShort(getString(R.string.login_expired))
